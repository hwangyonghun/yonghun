document.addEventListener('DOMContentLoaded', () => {
    ...
    async function handlePaymentSuccess(providerName) {
        paymentModal.classList.add('hidden');

        // Use the existing download logic but with paid flag
        const originalText = payTossBtn.innerHTML; // Store original state if needed
        ...
        notification.innerHTML = `
            <div class="payment-status-card">
                <i class="fa-solid fa-check-circle" style="color: #10b981; font-size: 3rem; margin-bottom: 1rem;"></i>
                <h3>${providerName} Payment Approved!</h3>
                <p>Generating your premium certificate...</p>
                <div class="loader" style="margin: 1rem auto;"></div>
            </div>
        `;
        document.body.appendChild(notification);

        // Add styles for this dynamic element if not exists
        if (!document.getElementById('payment-notify-style')) {
            const style = document.createElement('style');
            style.id = 'payment-notify-style';
            style.textContent = `
        ...
        try {
            // Call the API with paid=true
            const lang = document.getElementById('language-select').value;
            const response = await fetch(`/api/certificate?paid=true&lang=${lang}`);
            ...
            if (response.ok) {
                ...
                if (!pdfWindow) {
                    ...
                    a.style.display = 'none';
                    a.href = url;
                    a.download = 'Toubina_Premium_Certificate.pdf';
                    document.body.appendChild(a);
                    a.click();
                }

                // Success feedback
                const statusCard = notification.querySelector('.payment-status-card');
                statusCard.innerHTML = `
                    <i class="fa-solid fa-file-pdf" style="color: #ef4444; font-size: 3rem; margin-bottom: 1rem;"></i>
                    <h3>Certificate Downloaded!</h3>
                    <p>Thank you for using Toubina.</p>
                    <button id="close-notify-btn" class="cta-btn" style="margin-top:1rem; width:100%;">Close</button>
                `;
            ...
            console.error(error);
            alert('Error generating premium certificate. Please contact support.');
            notification.remove();
        }
    }
    ...
    payTossBtn.addEventListener('click', () => {
        const msg = getT('pay_confirm_toss');
        const confirmPayment = confirm(msg);

        if (confirmPayment) {
            payTossBtn.disabled = true;
            payTossBtn.innerHTML = '<span class="loader"></span> Processing...';

            setTimeout(() => {
                const approveMsg = getT('pay_approve');
                const success = confirm(approveMsg);

                if (success) {
                    handlePaymentSuccess('Toss Payments');
                }

                payTossBtn.disabled = false;
                payTossBtn.innerHTML = `
